        ...
        private static void CastHighPrioritySpells()
        {
            ...
            switch (ObjectManager.Player.ChampionName)
            {
                ...
                case "DrMundo":
                    ...
                    {
                        var targetHero = _GameInfo.Target;
                        if (Champdata.W.IsReady() && !WActive && enemyaround)
                        {
                            Champdata.W.Cast();
                        }
                        ...
                        if (Champdata.R.IsReady() && enemyaround && player.HealthPercent < 80)
                        {
                            Champdata.R.Cast();
                        }
                        return;
                    ...
                    if (_GameInfo.GameState == State.Jungling || _GameInfo.GameState == State.LaneClear)
                    {
                        ...
                        if (Champdata.R.IsReady() && targetMob.IsValidTarget(425) && player.HealthPercent < 80)
                        {
                            Champdata.R.Cast();
                        }
                        return;
